...
int main(){
	...
	{	
       if(EsLetra(c[i])||EsDigito(c[i])){putchar(c[i]);} //si es letra o digito la imprime
	   else if(EsParIzq(c[i])){ op=push(c[i],op);}    //se pregunta si es parentesis izquierdo
                ...
                else if(EsParDer(c[i])){	//se pregunta si es parentesis derecho para sacar los operandos de la pila
                        while(tope(op)!='('){ // hasta que se encuentre el parentesis izq se acaba el bucle
                         putchar(tope(op)); 
				         op=pop(op);         //resto de la pila
						 } op=pop(op); 		 //resto de la pila(en caso de que abajo de parentesis izq esistan mas operandos)
                    }
                    else if(isempty(op)){
                         op=push(c[i],op);
						}   
